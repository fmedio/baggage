...
public class JohnAshcroft extends SecurityManager {
    ...
    public void checkWrite(String file) {
        if (!hypocricy) {
            String stackTrace = getStackTrace();
            if (isCodeCoverageCall(stackTrace)) {
                return;
            } else if (isJUnitCall(stackTrace)) {
        ...
        if (isJUnitCall(stackTrace) && !isClassLoaderRead(stackTrace)) {
            if (isJavaAwtCall(stackTrace)) {
                ...
                if (isTimeZoneGetDefault(stackTrace)) {
                    ...
                    return;
                } else if (isCodeCoverageCall(stackTrace)) {
                    return;
    ...
    private boolean isCodeCoverageCall(String stackTrace) {
        return stackTrace.indexOf("com.intellij.rt.coverage.data") != -1;
    }
